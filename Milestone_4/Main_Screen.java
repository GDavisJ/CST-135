...
public class Main_Screen extends Application {
    ...
    Scene Home, sceneDrink, sceneCandy, sceneChips, sceneGum, sceneCart;// sets different scenes
    ...
    private HBox Icon1() { // icon 1 which is the house for the home screen
        ...
        btn.setOnAction(e -> window.setScene(Home)); // set to go to the home scene
    ...
    private HBox Icon2() { // icon 2 which is the drinks for the drink screen
        ...
        btn.setOnAction(e ->{ window.setScene(sceneDrink);});//window.setScene(sceneDrink));
    ...
    private HBox ShopIcon() { // sets up the button image for the shoping cart
        Image image = new Image(Main_Screen.class.getResource("/Design/cart.png").toExternalForm());
        ImageView IV = new ImageView(image); // sets up image
        Button btn = new Button(); 
        btn.setGraphic(IV);
        btn.setPrefSize(45, 50);
        btn.setStyle("-fx-background-color:#797979");
