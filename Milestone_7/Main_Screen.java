...
public class Main_Screen extends Application {
    ...
    Scene LOGIN, Home, sceneDrink, sceneCandy, sceneChips, sceneGum, sceneCart, sceneAdmin;// sets different scenes
    String Mainname;
    ...
    public void start(Stage primaryStage) {
        ...
        login.setCenter(customerIN());
        
        LOGIN = new Scene(login, 1350, 850);
        //****************** ********* ********* ********* ********* ********* ********* 
        window.setResizable(false); // the user cannot resize the frame
        window.setScene(LOGIN); // sets the first scene to Home
        window.show(); // show the window
        //****************** ********* ********* ********* ********* ********* ********* 
    }
    
  //****************** ********* ********* ********* ********* ********* ********* 
    public StackPane customerIN(){
        StackPane area = new StackPane();
        area.setStyle("-fx-background-color:#FFFFFB");
        Image background = new Image(Main_Screen.class.getResource("/Design/logback.jpg").toExternalForm());
        ImageView IV = new ImageView(background); // set up image
        ...
        user.setMaxSize(800,1500);
        user.setFont(Font.font("Sans-serif", FontWeight.NORMAL, FontPosture.REGULAR, 100));
        Log.add(user, 0, 2);


        Button Enter = new Button("Login");
        Enter.setOnAction(e -> { //FORCE LOADS ALL SCENES AT THIS POINT - helps to add in username
            Mainname = user.getText();
        ...
        ImageView IVH = new ImageView(image); // set up image
        IVH.setFitWidth(800);;
        home.setCenter(IVH);
        ...
        Home = new Scene(home, 1350, 850); // sets the Home scene to the home borderpane
        sceneDrink = new Scene(drinkPage, 1350, 850); // sets the Drink scene to the drink borderpane
        sceneCandy = new Scene(candyPage, 1350, 850); // sets the Candy scene to the candy borderpane
        sceneChips = new Scene(chipsPage, 1350, 850); // sets the Chips scene to the chips borderpane
        sceneGum = new Scene(gumPage, 1350, 850); // sets the Gum scene to the gum borderpane
        sceneCart = new Scene(cartPage, 1350, 850); // sets the cart scene to the cart borderpane
        sceneAdmin = new Scene(adminPage, 1350, 850); // sets the cart scene to the cart borderpane
        
        window.setResizable(false); // the user cannot resize the frame
        window.setScene(Home); // sets the first scene to Home
    });
        Enter.setMaxSize(400, 800);
        Enter.setFont(Font.font("Sans-serif", FontWeight.NORMAL, FontPosture.REGULAR, 70));
        Log.add(Enter, 0, 4);
        
        area.getChildren().add(Log);
        return area;
    }
    ...
    private HBox Title() {
        ...
        Text name = new Text("Welcome ");//+ Mainname);
        name.setFont(Font.font("Sans-serif", FontWeight.NORMAL, FontPosture.REGULAR, 20));
        name.setFill(Color.rgb(255, 255, 255));
    ...
    private StackPane animationMeth() {
	    ...
	    PathTransition pt = new PathTransition(Duration.millis(2000), new Line(1083,-85,1083,-70), rect);
	    pt.setCycleCount(1);
	    pt.play();
	    return stkPane;
    }
  ...
  //Used to create GridPane for the admin
    private  GridPane gridPaneAdmin(Object[] itemArr) {
    	...
    	//Use a manage button to open the manager window.
    	Button btnManager = new Button("Manager Access");
    	gridPaneobj.add(btnManager, 4, btnCnt);
   		GridPane.setHalignment(btnManager, HPos.CENTER);
   			btnManager.setOnAction(eve -> {
   			Stage menuStage = new Stage();
   			Manage_Main menu = new Manage_Main();
